...
export default function Log_Tanker() {

    const { current_tanker_logs } = tankers_selectors()
            ...
            <Container>
                ...
                <div className="w-full h-[90vh] overflow-y-scroll visible_thumb_bar">
                    <table className={`w-full h-[${10 * logs.length}]`}>
                        ...
                        <tbody>
                            {current_tanker_logs?.map((log, index) => (<tr className="text-[#800E80] text-[14px] text-center font[400]" key={index}>
                                <td>{index}</td>
